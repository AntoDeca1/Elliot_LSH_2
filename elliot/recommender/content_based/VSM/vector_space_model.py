...
class VSM(RecMixin, BaseRecommenderModel):
    ...
    def __init__(self, data, config, params, *args, **kwargs):
        ...
        self.autoset_params()
        # DA AGGIUSTARE IL FATTO DI ITEMATTRIBUTES MEGLIO
        # Questa riga è stata aggiunta
        self._loader = config.data_config.side_information[0].dataloader
    ...
    def compute_binary_profile(self, user_items_dict: t.Dict):
        """
        1)Prende il dizionario contente gli elementi recensiti dall'utente con i rispettivi rating
        2)Prende le chiavi di questo dizionario (id degli item)
        3)Per ogni item
         -3a) Prende la lista di feature associate(id numerici)
         -3b) Li aggiunge al dizionario user features con un 1 accanto
        4)In output avremo la lista di tutti i generi(?) dell'
